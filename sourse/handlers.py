...
def start(bot, update):
    ...
    bot.message.reply_text(f'{bot.message.chat.first_name}, Вас приветствует Foodbot. '
                           f'Прежде чем начать работу, мне нужно узнать кое-что о вас. '
                           f'Давайте познакомимся!', reply_markup=keyboards())


def acquaintance(bot, update):
    bot.message.reply_text('Как к Вам обращаться? Введите имя или псевдоним: ', reply_markup=ReplyKeyboardRemove())
    return 'user_name'
...
def get_user_goal(bot, update):
    ...
    bot.message.reply_text(f'Отлично, {update.user_data["name"].capitalize()}! '
