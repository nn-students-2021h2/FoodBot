...
def main():
    bot = Updater(token='5038288042:AAHIZfCj2HqCmUlTrVMt5oQU5TmHAL9Fcco', use_context=True)
    dispatcher = bot.dispatcher

    dispatcher.add_handler(CommandHandler('start', start))
    dispatcher.add_handler(
        ConversationHandler(entry_points=[MessageHandler(Filters.regex('Познакомиться'), acquaintance)],
                            states={
                                'user_name': [MessageHandler(Filters.text, get_user_name)],
                                'user_birth_date': [MessageHandler(Filters.text, get_user_birth_date)],
                                'user_sex': [MessageHandler(Filters.regex('Мужской|Женский'), get_user_sex)],
