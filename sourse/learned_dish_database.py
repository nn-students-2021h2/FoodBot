...
def add_learned_dish_note(
    ...
    try:
        ...
        con.rollback()
        print(
            f"ERROR of adding '{learned_dish_dish}' in learned_dish database - pymysql {e.args[0]}: {e.args[1]}"
        )
    cursor.close()
    con.close()
...
def delete_learned_dish_note(learned_dish_dish: str) -> None:
    ...
    try:
        ...
        con.rollback()
        print(
            f"ERROR of deleting '{learned_dish_dish}' from learned_dish database - pymysql {e.args[0]}: {e.args[1]}"
        )
    cursor.close()
    con.close()
...
def get_learned_dish_note(learned_dish_dish: str) -> dict | None:
    ...
    try:
        ...
        print(
            f"ERROR of getting '{learned_dish_dish}' from learned_dish database - pymysql {e.args[0]}: {e.args[1]}"
        )
    cursor.close()
    con.close()
